...
test('isolamento: runtime e migrations não referenciam produção ou secrets',async()=>{
  const root=join(dirname(fileURLToPath(import.meta.url)),'..')
  const migrationFiles=(await readdir(join(root,'migrations'))).filter(f=>f.endsWith('.sql')).sort()
  const files=['src/core.js','src/server.js','public/index.html',...migrationFiles.map(f=>`migrations/${f}`)]
  const content=(await Promise.all(files.map(f=>readFile(join(root,f),'utf8')))).join('\n')
...
test('nova versão de regra não altera comissão histórica',()=>{
  ...
  assert.equal(before,3000);assert.equal(service.snapshot(admin).commissionEntries[0].amountCents,3000)
})
