...
const financialDateIso=(v:unknown)=>{
  const raw=String(v??"").trim();
  if(!raw)return null;
  if(/^\d{4}-\d{2}-\d{2}$/.test(raw))return new Date(`${raw}T00:00:00-03:00`).toISOString();
